...
const double Wav_File_Work::zcrHigh = 10;                                    //初始化短时过零率高门限
const double Wav_File_Work::zcrLow = 5;                                      //初始化短时过零率低门限
...
bool Wav_File_Work::Frame_Energy(void)                                       //用于求短时帧能量
{
	...
	double sum = 0;
	for (unsigned long i = 0; i < Wav_File::Get_dataNumber() - N; ++i) {     //这是所有短时帧能量数据的个数
...
{
	if (Number >= (Wav_File::Get_dataNumber())) {                            //如果所需要的数超过了数据个数
...
{
	if (Number >= (Wav_File::Get_dataNumber() - N)) {                        //如果所需要的数超过了数据个数
		MessageBoxA(NULL, "ERROR : Over list !", "ERROR", MB_ICONHAND);
		throw invalid_argument("ERROR : Over list !");
		return -1;
	}
...
{
	if (Number >= voiceNumber) {                                             //如果所需要的数超过了数据个数
		MessageBoxA(NULL, "ERROR : Over list !", "ERROR", MB_ICONHAND);
		throw invalid_argument("ERROR : Over list !");
	}
	else {
...
bool Wav_File_Work::Endpoint_Detection(void)                                 //端点检测函数
{
	...
	for (unsigned long i = 0; i < Wav_File::Get_dataNumber() - N; ++i) {     //遍历每一帧
		switch (statusFlag)
		{
		case MUTEPARAGRAPH:
		case INTERIMPARAGRAPH:
			if (Get_DataEnergy(i) > energyHigh) {                            //帧能量大于能量高门限,进入语音段
				begin = max(i - voiceLength - 1, 0);
				statusFlag = VOICEPARAGRAPH;
				voiceLength++;
				silence = 0;
			}
			else if (Get_DataEnergy(i) > energyLow || Get_DataZCR(i) > zcrLow) {       //过渡段
			...
			break;
		case VOICEPARAGRAPH:
			if (Get_DataEnergy(i) > Get_minEnergy() || Get_DataZCR(i) > Get_minZCR()) {//保持在语音段
				voiceLength++;
			}
			else {                                                           //语音将结束
				silence++;
