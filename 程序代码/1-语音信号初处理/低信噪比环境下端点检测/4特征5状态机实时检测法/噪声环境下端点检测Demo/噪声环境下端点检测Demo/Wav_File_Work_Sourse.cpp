...
void Wav_File_Work::ShowData(void)                                           //覆盖父类的展示数据函数
{
	...
	*/
	cout << "倒谱距离数据：" << endl;
	unsigned long *showingData = new unsigned long[101];
	for (int i = 0; i < 100; ++i) {
		showingData[i] = 0;
	}
	for (unsigned long i = 0; i < frameNumber; ++i) {
		showingData[abs((int)(dataCepsDistance[i] * 100))]++;
	}
	for (int i = 0; i < 10; ++i) {
		cout << "0." << i << "0~0." << i << "9" << "   : ";
		int sum_t = 0;
		for (int j = 0; j < 10; ++j) {
			cout << showingData[i * 10 + j] << "\t";
			sum_t += showingData[i * 10 + j];
		}
		cout << " | " << sum_t << endl;
	}
	...
	for (unsigned long i = 0; i < frameNumber; ++i) {
		showingData[abs((int)(dataEngChart[i] * 100))]++;
	}
	for (int i = 0; i < 10; ++i) {
		cout << "0." << i << "0~0." << i << "9" << "   : ";
		int sum_t = 0;
	...
	for (unsigned long i = 0; i < frameNumber; ++i) {
		showingData[abs((int)(dataEngDevia[i] * 100))]++;
	}
	for (int i = 0; i < 10; ++i) {
		cout << "0." << i << "0~0." << i << "9" << "   : ";
		int sum_t = 0;
	...
	for (unsigned long i = 0; i < frameNumber; ++i) {
		showingData[abs((int)(dataSpectrumEnt[i] * 100))]++;
	}
	for (int i = 0; i < 10; ++i) {
		cout << "0." << i << "0~0." << i << "9" << "   : ";
		int sum_t = 0;
