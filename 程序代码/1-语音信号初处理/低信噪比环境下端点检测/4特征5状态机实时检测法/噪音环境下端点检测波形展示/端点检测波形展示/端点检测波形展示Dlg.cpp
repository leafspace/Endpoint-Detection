...
void C端点检测波形展示Dlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_LIST1, ctrlList_paragraph);                          //初始化列表
...
BEGIN_MESSAGE_MAP(C端点检测波形展示Dlg, CDialogEx)
	...
	ON_BN_CLICKED(IDC_BUTTON3, &C端点检测波形展示Dlg::OnBnClickedButton3)
END_MESSAGE_MAP()
...
BOOL C端点检测波形展示Dlg::OnInitDialog()
{
	...
	CRect rect;
	ctrlList_paragraph.GetHeaderCtrl()->EnableWindow(false);                   //固定标题不被移动
	ctrlList_paragraph.GetClientRect(&rect);                                   //获取编程语言列表视图控件的位置和大小
	ctrlList_paragraph.SetExtendedStyle(ctrlList_paragraph.GetExtendedStyle()
		| LVS_EX_FULLROWSELECT | LVS_EX_GRIDLINES);                            //为列表视图控件添加全行选中和栅格风格

	ctrlList_paragraph.InsertColumn(0, _T("No"), LVCFMT_CENTER, 30, 0);
	ctrlList_paragraph.InsertColumn(1, _T("Begin"), LVCFMT_CENTER, (rect.Width() - 30) / 3, 1);
	ctrlList_paragraph.InsertColumn(2, _T("End"), LVCFMT_CENTER, (rect.Width() - 30) / 3, 2);
	ctrlList_paragraph.InsertColumn(3, _T("Length"), LVCFMT_CENTER, (rect.Width() - 30) / 3, 3);
...
void C端点检测波形展示Dlg::StartDraw(CRect rc, CDC* pdc, int flag, int Flag)
{
	...
	//MemDC.FillSolidRect(0, 17, width, height, RGB(255, 255, 255));           //先用背景色将位图清除干净
	...
	if (Flag == 1 || Flag == 3) {                                              //绘制横坐标
		pOldPen = MemDC.SelectObject(new CPen(PS_SOLID, 1, RGB(0, 0, 0)));                                     //选择新画笔，并将旧画笔的指针保存到pOldPen
...
void C端点检测波形展示Dlg::Plot1(CRect rc, CDC* pdc, int flag)
{
	...
	*/
	...
	for (unsigned long i = 0; i < File->Get_voiceNumber(); ++i) {
		...
		pOldPen = pdc->SelectObject(new CPen(PS_SOLID, 1, RGB(0, 0, 255)));                                     //选择新画笔，并将旧画笔的指针保存到pOldPen
		pdc->MoveTo(begin, 17);
		pdc->LineTo(begin, height - 5);
		pOldPen = pdc->SelectObject(new CPen(PS_SOLID, 1, RGB(0, 255, 0)));                                     //选择新画笔，并将旧画笔的指针保存到pOldPen
		pdc->MoveTo(end, 17);
		pdc->LineTo(end, height - 5);
...
void C端点检测波形展示Dlg::ShowParagraphList()
{

	ctrlList_paragraph.DeleteAllItems();

	//在列表视图控件中插入列表项，并设置列表子项文本
	wchar_t *tempStr = new wchar_t[50];
	for (unsigned long i = 0; i < File->Get_voiceNumber(); ++i) {
		VoiceParagraph tempPoint = File->Get_dataVoicePoint(i);
		swprintf(tempStr, L"%d", (i + 1));
		ctrlList_paragraph.InsertItem(i, tempStr);
		swprintf(tempStr, L"%d", tempPoint.begin);
		ctrlList_paragraph.SetItemText(i, 1, tempStr);
		swprintf(tempStr, L"%d", tempPoint.end);
		ctrlList_paragraph.SetItemText(i, 2, tempStr);
		swprintf(tempStr, L"%d", tempPoint.voiceLength);
		ctrlList_paragraph.SetItemText(i, 3, tempStr);
	}
	swprintf(tempStr, L"Paragraph：%d", (int)File->Get_voiceNumber());
	GetDlgItem(IDC_STATIC6)->SetWindowText(tempStr);
}
...
void C端点检测波形展示Dlg::OnBnClickedButton1()
{
	...
	if ((fp = fopen(fileNameTemp, "rb")) == NULL) {
		...
		MessageBoxA(NULL, "ERROR : You have not selected the file !", "ERROR", MB_ICONHAND);
		return;
	}
...
void C端点检测波形展示Dlg::OnBnClickedButton2()
{
	...
	if (this->fp == NULL) {
		...
		if (opendlg.DoModal() == IDOK)
		{
			...
			File->SaveNewWav(fileNameTemp);
			MessageBoxA(NULL, "TIP : Voice created ok !", "TIP", MB_ICONASTERISK);
		}
	}
}

void C端点检测波形展示Dlg::OnBnClickedButton3()
{
	// TODO: 在此添加控件通知处理程序代码
	if (this->fp == NULL) {
		MessageBoxA(NULL, "ERROR : No File !", "ERROR", MB_ICONHAND);
		...
		CFileDialog opendlg(FALSE, _T("*.txt"), _T("*.txt"), OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT, _T("所有文件(*.txt*;)|*.txt*||"), NULL);   //打开文件选择框
		if (opendlg.DoModal() == IDOK)
		{
			CString fileName = opendlg.GetPathName();                          //获取选择的文件名
			int nameLen = WideCharToMultiByte(CP_ACP, 0, fileName, -1, NULL, 0, NULL, NULL);
			...
			File->SaveParagraphInfo(fileNameTemp);
			MessageBoxA(NULL, "TIP : Parahraph information saved ok !", "TIP", MB_ICONASTERISK);
		}
	}
}
