...
bool Wav_File::Read_File(void)
{
	try
	{
		...
		id_FMT[3] = 32;
		id_FMT[4] = 0;
		fread(&formatLength, sizeof(unsigned long), 1, fp);                    //采样频率，单位khz
		fread(&formatTag, sizeof(short), 1, fp);                               //读取文件标签
		fread(&channelsNumber, sizeof(short), 1, fp);                          //读取通道数目
		fread(&sampleRate, sizeof(unsigned long), 1, fp);                      //读取采样频率，单位hz
...
void Wav_File::SaveNewWav(unsigned long voiceNumber, vector<VoiceParagraph> voiceParagraph, int frameShift, char* path)      //保存去掉空白处的语音文件，参数1为段落的个数，参数2为各个语音段落的开始点与结束点信息
{
	...
	unsigned long removerLength = dataNumber - endPointLength *frameShift;
	...
	try
	{
		...
		unsigned long n = 0;
		for (unsigned long i = 0; i < voiceNumber; ++i) {                      //逐段落写入语音端点信息
			VoiceParagraph tempParagraph = voiceParagraph[i];
			tempParagraph.begin *= frameShift;
			tempParagraph.end *= frameShift;
