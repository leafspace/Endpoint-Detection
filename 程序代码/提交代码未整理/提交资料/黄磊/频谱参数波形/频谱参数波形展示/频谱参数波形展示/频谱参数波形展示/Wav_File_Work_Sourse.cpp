...
{
	if (n>= (Wav_File_Work::Get_XnNumber()) || n < 0) {              //如果所需要的数超过了数据个数
...
{
	if (n >= (Wav_File_Work::Get_dataDFTNumber()) || n < 0) {              //如果所需要的数超过了数据个数
		MessageBoxA(NULL, "ERROR : Over list !", "ERROR", MB_ICONHAND);
		throw invalid_argument("ERROR : Over list !");
		return -1;
	}
...
bool Wav_File_Work::IDFT(double *data, unsigned long dataNumber)             //逆向离散傅立叶变换，无虚部
{
	double exp;                                                              //这是指数部分
	double *dataIDFT;
	dataIDFT = (double*)malloc(sizeof(double)*dataNumber);                   //为新数据分配空间
	if (dataIDFT == NULL) {                                                  //分配失败的情况
		throw invalid_argument("ERROR : Memory failure !");
		return false;
	}
...
{
	if (n >= (Wav_File_Work::Get_dataIDFTNumber()) || n < 0) {              //如果所需要的数超过了数据个数
		MessageBoxA(NULL, "ERROR : Over list !", "ERROR", MB_ICONHAND);
		throw invalid_argument("ERROR : Over list !");
		return -1;
	}
...
bool Wav_File_Work::FFT(void)
{
	...
	M = (int)(log(Wav_File::Get_dataNumber()) / log(2));
	...
	for (l = 1; l <= M; l++) {   /* loop for ceil{log2(N)} */
		...
		for (j = 1; j <= le2; j++) {   /* loop for each sub DFT */
			...
			for (i = j - 1; i <= N - 1; i += le) {  /* loop for each butterfly */
				ip = i + le2;
				tR = dataFFT[ip] * uR - dataFFT[ip] * uI;
				tI = dataFFT[ip] * uI + dataFFT[ip] * uR;
				dataFFT[ip] = dataFFT[i] - tR;
				dataFFT[ip] = dataFFT[i] - tI;
				dataFFT[i] += tR;
				dataFFT[i] += tI;
			}  /* Next i */
			tR = uR;
...
{
	if (n >= (Wav_File_Work::Get_dataFFTNumber()) || n < 0) {              //如果所需要的数超过了数据个数
		MessageBoxA(NULL, "ERROR : Over list !", "ERROR", MB_ICONHAND);
		throw invalid_argument("ERROR : Over list !");
		return -1;  
	}
...
{
	if (n >= (Wav_File_Work::Get_EmNumber()) || n < 0) {              //如果所需要的数超过了数据个数
		MessageBoxA(NULL, "ERROR : Over list !", "ERROR", MB_ICONHAND);
		throw invalid_argument("ERROR : Over list !");
		return -1;
	}
...
{
	if (n >= (Wav_File_Work::Get_SCmNumber()) || n < 0) {              //如果所需要的数超过了数据个数
		MessageBoxA(NULL, "ERROR : Over list !", "ERROR", MB_ICONHAND);
		throw invalid_argument("ERROR : Over list !");
		return -1;
	}
...
{
	if (n >= (Wav_File_Work::Get_BWmNumber()) || n < 0) {              //如果所需要的数超过了数据个数
		MessageBoxA(NULL, "ERROR : Over list !", "ERROR", MB_ICONHAND);
		throw invalid_argument("ERROR : Over list !");
		return -1;
	}
...
{
	if (n >= (Wav_File_Work::Get_AiNumber()) || n < 0) {              //如果所需要的数超过了数据个数
		MessageBoxA(NULL, "ERROR : Over list !", "ERROR", MB_ICONHAND);
		throw invalid_argument("ERROR : Over list !");
		return NULL;
	}
...
{
	if (n >= (Wav_File_Work::Get_HNumber()) || n < 0) {              //如果所需要的数超过了数据个数
		MessageBoxA(NULL, "ERROR : Over list !", "ERROR", MB_ICONHAND);
		throw invalid_argument("ERROR : Over list !");
		return -1;
	}
...
{
	int i, n;
	hSpectrum = (double*)malloc(sizeof(double)*Wav_File::Get_dataNumber() - N);//为Double型数据申请内存空间
	if (hSpectrum == NULL) {												 //分配空间未成功
		throw invalid_argument("ERROR : Memory failure !");
	}
	for (i = 0, n = 0; i < Wav_File::Get_dataNumber() - N; i += Wav_File_Work::FrameShift, n++)
	{
...
{
	if (n >= (Wav_File_Work::Get_hSpectrumNumber()) || n < 0) {              //如果所需要的数超过了数据个数
		MessageBoxA(NULL, "ERROR : Over list !", "ERROR", MB_ICONHAND);
		throw invalid_argument("ERROR : Over list !");
		return -1;
	}
